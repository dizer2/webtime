...
const ServicesPage = ({ currentPage, hideLoader, setHideLoader }) => {
    ...
    const handleScrollToSection = (sectionId) => {
      if (isMenuOpen) {
        ...
        document
          ...
          .classList.add("_quickly-close-overlay");
      }
    ...
    const menuHandleClick = () => {
      ...
      document
        .querySelector(".header__menu-overlay")
        .classList.remove("_quickly-close-overlay");
	...
	const [isSendSuccessfully, setIsSendSuccessfully] = useState(false);
  ...
  const sendEmail = (e) => {
    ...
    // Add service_name to formData
    const formDataWithServiceName = {
      ...formData,
    ...
    emailjs
      .send('service_pk5t8sv', 'template_nps1hmm', {
        name: `${formData.name}`,
      ...
      .then(
        () => {
          ...
          document.querySelectorAll('.services-page__popup-input').forEach(input => input.value = '');
        },
        (error) => {
    ...
    <section
      ...
      style={{
        backgroundImage: `url(${t(
          `servicesPage.${currentPage}.backgroundImg`
        )})`,
      }}
    >

      <Popup popupClass={isSendSuccessfully} setPopupClass={setIsSendSuccessfully}/>
      ...
      <div
        className={`services-page__popup-overlay ${
          togglePopup ? "_active" : ""
        }`}
      >
        <div className={`services-page__popup`}>
          <div className="services-page__popup-header">
            <p className="services-page__popup-title">
              {t("servicesPage.main.contactBtn")}
            </p>
            ...
            >
              <g clipPath="url(#clip0_1016_357)">
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M11.446 10.0004L20 18.147L18.553 19.5242L10 11.3775L1.44601 19.5242L0 18.147L8.553 10.0004L0 1.8537L1.44601 0.476562L10 8.62323L18.553 0.476562L20 1.8537L11.446 10.0004Z"
                  fill={`url(#${t(
                    `servicesPage.${currentPage}.gradientParts.gradientID`
                  )})`}
                />
              </g>
              <defs>
                <linearGradient
                  id="paint0_linear_1016_357"
                  x1="0.904161"
                  y1="3.22934"
                  x2="22.0969"
                  y2="6.69258"
                ...
                >
                  <stop
                    stopColor={t(
                      `servicesPage.${currentPage}.gradientParts.color1`
                    )}
                  />
                  <stop
                    offset={t(
                      `servicesPage.${currentPage}.gradientParts.offset`
                    )}
                    stopColor={t(
                      `servicesPage.${currentPage}.gradientParts.color2`
                    )}
                  />
                  <stop
                    offset="1"
                    stopColor={t(
                      `servicesPage.${currentPage}.gradientParts.color3`
                    )}
                  />
                </linearGradient>
          ...
          <form onSubmit={sendEmail} ref={form}  className="services-page__popup-form">
            <div className="services-page__popup__input-box">
              <span>{t("servicesPage.popup.name")} <span className={`${error.name && 'error'}`}>{error.name && 'error' ? t("servicesPage.popup.inputNameEmpty") : ''}</span></span>
              <input
                className="services-page__popup-input"
                onChange={handleChange}
                type="text"
                placeholder={t("servicesPage.popup.namePlaceholder")}
                name="name"
              />
            </div>
            <div className="services-page__popup__input-box">
              {/* <span className={`${error.name && 'error'}`}>{t("servicesPage.popup.phone")}</span> */}
              <span>{t("servicesPage.popup.phone")} <span className={`${error.name && 'error'}`}>{error.name && 'error' ? t("servicesPage.popup.inputPhoneEmpty") : ''}</span></span>

              <input
                className="services-page__popup-input"
                onChange={handleChange}
                type="text"
                placeholder={t("servicesPage.popup.phonePlaceholder")}
                name="phone"
              />
            </div>
            <div className="services-page__popup__input-box">
              {/* <span className={`${error.name && 'error'}`}>{t("servicesPage.popup.email")}</span> */}
              <span>{t("servicesPage.popup.email")} <span className={`${error.email && 'error'}`}>{error.email && 'error' ? t("servicesPage.popup.emailIsntValid") : ''}</span></span>

              <input
                className="services-page__popup-input"
                onChange={handleChange}
                type="text"
                placeholder={t("servicesPage.popup.emailPlaceholder")}
                name="email"
              />
            </div>
            <div className="services-page__popup__input-box">
              {/* <span className={`${error.name && 'error'}`}>{t("servicesPage.popup.message")}</span> */}
              <span>{t("servicesPage.popup.message")} <span className={`${error.message && 'error'}`}>{error.message && 'error' ? t("servicesPage.popup.inputMessageEmpty") : ''}</span></span>

              <div className="services-page__popup__textarea-wrapper">
                <textarea
                  className="services-page__popup-input services-page__popup-textarea"
                  onChange={handleChange}
                  placeholder={t("servicesPage.popup.messagePlaceholder")}
                    name="message"
                ></textarea>
              </div>
            </div>
      ...
      <div
        className={`services-page__menu ${
          scrollingDown ? "_menu-hide" : "_menu-show"
        } ${isUserOnTop || isMenuOpen ? "_hide-bg" : ""}`}
      >
        <div className="services-menu__wrapper">
          <Link
            ...
            className="services-page__menu-logo--link"
          >
              ...
              <div className="social__media-wrapper">
                ...
                <SocialMedia
                  ...
                  icon={
                    ...
                    >
                      <path
                        d="M10.0385 38.3703H15.5972V24.8707L7.65625 18.915V35.988C7.65625 37.3062 8.72431 38.3703 10.0385 38.3703Z"
                        fill="white"
                      />
                      <path
                        d="M34.6562 38.3703H40.2149C41.5331 38.3703 42.5972 37.3023 42.5972 35.988V18.915L34.6562 24.8707"
                        fill="white"
                      />
                      <path
                        d="M34.6562 14.5477V24.8709L42.5972 18.9152V15.7389C42.5972 12.7928 39.2342 11.1133 36.8797 12.8801"
                        fill="white"
                      />
                      <path
                        d="M15.5977 24.8706V14.5474L25.1268 21.6942L34.656 14.5474V24.8706L25.1268 32.0174"
                        fill="white"
                      />
                      <path
                        d="M7.65625 15.7389V18.9152L15.5972 24.8709V14.5477L13.3738 12.8801C11.0153 11.1133 7.65625 12.7928 7.65625 15.7389"
                        fill="white"
                      />
                    </svg>
                  }
      ...
      <div className="services-page__main">
        ...
        <p className="services-page__subtitle">
          {t(`servicesPage.${currentPage}.subtitle`)}
        </p>
        ...
        {Object.values(t(`servicesPage.${currentPage}.services`)).map(
          (block, blockIndex) => (
            <div
              key={blockIndex}
              id={block.blockID}
              className="services-page__description-box"
            >
              <div className="services-page__description-title">
                <span
                  style={{
                    background: t(`servicesPage.${currentPage}.mainColor`),
                  }}
                ></span>
                <p>{block.title}</p>
                <span
              ...
              <p className="services-page__description">{block.description}</p>

              <ArrowBtn
                handleClick={() => handlePopupClick(block.title)}
                text={t(`servicesPage.main.contactBtn`)}
                customGradient={{
                  direction: `${
                    t(`servicesPage.${currentPage}.gradientParts.offset`) * 10
                  }deg`,
                  colors: [
                    t(`servicesPage.${currentPage}.gradientParts.color1`),
                    t(`servicesPage.${currentPage}.gradientParts.color2`),
                    t(`servicesPage.${currentPage}.gradientParts.color3`),
                  ],
                }}
                gradientID={t(
                  `servicesPage.${currentPage}.gradientParts.gradientID`
